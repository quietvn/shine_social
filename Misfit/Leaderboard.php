...
class MisfitLeaderboard extends MisfitDbModelAbstract {
	...
	public function getLeaderboard2($id_exp, $id_group = 0) {
		...
		$query = "
			SELECT *, group_exps.id_twitter AS exp_twitter, users.id_twitter AS user_twitter FROM leaderboard
			INNER JOIN users 
			...
			$where
			ORDER BY leaderboard.id_group, users.id, points DESC
		";
		...
		foreach ($users as $user) {
			...
			$result[ $user['id_group'] ] [ $user['id_user'] ] ['exp_twitter'] = $user['exp_twitter'];
